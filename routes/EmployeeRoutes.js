...
app.get('/employees', async (req, res) => {
  ...
  //get all docs
  //const employees = await employeeModel.find({}); // When you search without any criteria you will get all of the documents

  //Sorting
  //use "asc", "desc", "ascending", "descending", 1, or -1
  //const employees = await employeeModel.find({}).sort({'firstname': -1}); // Sort by first name starting with Z
  
  //Select Specific Column: Want only 3 columns and want to sort by salaray descending
...
app.get('/employee', async (req, res) => {
  ...
  // const employees = await employeeModel.find({_id: req.query.id}); //returns array of objects - created to get you all the matching documents. Fiond always returns an array
  // const employees = await employeeModel.findById(req.query.id); // returns single document ; object (10 items in one object). findBy always return one single document/object
...
app.get('/employees/firstname/:name', async (req, res) => {
  ...
  //Using Static method
  const employees = await employeeModel.getEmployeeByFirstName(name) // method defined in Employee.js - make it modular and reusable
  
  //Using Query Helper
  // const employees = await employeeModel.findOne().byFirstName(name) // how is sequred in helper method - so just call method here
