...
const EmployeeSchema = new mongoose.Schema({
  firstname: {
    ...
    trim: true // Removed whitespace before and after the firstname
    
  },
  lastname: {
    type: String,
    alias: 'surname', // Nickname for lastname ; only for use in the app; not in the database
    required: [true, "Last name cannot be empty"], // Associated error message if the validation is violated
    trim: true
  },
  email: {
    type: String,
    ...
    trim: true,
    lowercase: true,
    minLength: 5,   // Can put this for any field
    maxLength: 50,   // Can put this for any field
  ...
  gender: {
    type: String,
    required: true,
    enum: ['male', 'female', 'non-binary', 'prefer not to say'], // Use when you only want to accept predefned values
    lowercase: true
  },
  city:{
    type: String,
...
//Declare Virtual Fields - only available in app while running - dont want to store in db
EmployeeSchema.virtual('fullname')
  .get(function(){
    return `${this.firstname} ${this.lastname}`
  })
  .set(function(value){
    // Temporary virtual variable 
...
//Writing Query Helpers - Mongoose provide you query functions
